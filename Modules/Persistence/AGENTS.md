...
Implementação concreta de `PersistenceInterfaces`: `UserDefaultsStore`, `KeychainStore`, `CoreDataStore<Entity: PersistentEntityProtocol, ManagedObject: NSManagedObject>` (motor genérico). Depende só de `PersistenceInterfaces`.
...
`CoreDataStore` é genérico: pra testar sem entidade de produto, monta um `NSManagedObjectModel` programático em memória (ver `Tests/PersistenceTests/Doubles/TestItemMock.swift`) — não usa o `DB.xcdatamodeld` real do módulo pra isso.
